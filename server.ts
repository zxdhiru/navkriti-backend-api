...
if (cluster.isPrimary) {
    ...
    cluster.on("exit", (worker, code, signal) => {
        ...
        if (workers.size === 0) {
            console.error(
                "All workers have exited. Stopping the primary process."
            );
            process.exit(1);
        }
    });
    ...
    if (!process.env.MONGO_URI || !process.env.MONGO_DB) {
        console.error(
            "❌ MONGO_URI or MONGO_DB environment variable is missing!"
        );
        process.exit(1);
    }
    ...
    const corsOptions = {
        origin: [
            "http://localhost:5173",
            "https://navkriti-frontend.onrender.com",
            "https://navkriti.onrender.com",
        ],
